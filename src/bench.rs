...
mod tests {
    extern crate chrono;
    use aggsig::{export_secnonce_single, sign_single, verify_single};
    ...
    fn bench_aggsig_check_with_pubnonce_efficiency() {
        ...
        println!("aggsig check ok:\t{}/{}", ok_count, LENGTH);
        println!(
            "spent time:\t{}ms/({} verify with pubnonce)",
            dur_ms, LENGTH
        );
    }
    #[test]
    ...
    fn bench_bullet_proof_wt_extra() {
        ...
        println!(
            "spent time:\t{}ms/({} verify bullet proof)",
            dur_ms, BP_LENGTH
        );
    }
    ...
    fn bench_bullet_proof_with_extra_msg() {
        ...
        println!(
            "spent time:\t{}ms/({} bullet proof with extra msg)",
            dur_ms, BP_LENGTH
        );
        ...
        println!("verify_bullet_proof ok:\t{}/{}", ok_count, BP_LENGTH);
        println!(
            "spent time:\t{}ms/({} verify bullet proof with extra msg)",
            dur_ms, BP_LENGTH
        );
    }
    ...
    fn bench_bulletproof_batch_verify() {
        ...
        println!(
            "spent time:\t{}ms/({} bullet proof creation w/o extra message)",
            dur_ms, BP_LENGTH
        );
        ...
        println!(
            "spent time:\t{}ms/(1 batch verify for {} bullet proofs w/o extra message)",
            dur_ms, BP_LENGTH
        );
    ...
    fn bench_bulletproof_with_extra_batch_verify() {
        ...
        println!(
            "spent time:\t{}ms/({} bullet proof creation w/ extra data)",
            dur_ms, BP_LENGTH
        );

        let start = Utc::now().timestamp_nanos();
        let proof_range =
            secp.verify_bullet_proof_multi(commits, proofs, Some(extra_data_vec.clone()));
        let fin = Utc::now().timestamp_nanos();
        let dur_ms = (fin - start) as f64 * NANO_TO_MILLIS;

        println!(
            "spent time:\t{}ms/(1 batch verify for {} bullet proofs w/ extra data)",
            dur_ms, BP_LENGTH
        );
        println!("\nproof_range:\t{:#x?}", proof_range.unwrap());
    }
    ...
    fn bench_generator_h_efficiency() {
        ...
        const GENERATOR_H_V2: [u8; 33] = [
            0x0a, 0x50, 0x92, 0x9b, 0x74, 0xc1, 0xa0, 0x49, 0x54, 0xb7, 0x8b, 0x4b, 0x60, 0x35,
        ...
        println!(
            "spent time:\t{}ms/({} pedersen commitment with H1)",
            dur_ms, PC_LENGTH
        );
        ...
        println!(
            "spent time:\t{}ms/({} pedersen commitment with H2)",
            dur_ms, PC_LENGTH
        );
    }
    ...
    fn bench_pubkey_check_eq() {
        ...
        println!(
            "spent time:\t{}ms/({} pubkey equal check)",
            dur_ms, MAX_LENGTH
        );
    }
